...
- A real `iris_cli.exe` to see anything past camera setup. Easiest path:
  ...
  <https://iris.cs.bath.ac.uk/>, then just `npm run dev`, no extra setup
  needed. See [IRIS_BUNDLING.md](IRIS_BUNDLING.md) for env var overrides,
  bundling for distribution, writing your own pipeline spec, or a gotcha
  around personal `IRIS_HOME` overrides shadowing the installer.
